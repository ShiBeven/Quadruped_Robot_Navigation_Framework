#include "nav2_plugins/bt/action/select_patrol_path.hpp"
...
{
  node_ = getNodeFromBlackboard(*this);
  ...
  node_->get_parameter("nav.goal_points.x", xs);
  node_->get_parameter("nav.goal_points.y", ys);
  node_->get_parameter("nav.goal_points.z", zs);
  node_->get_parameter("nav.point_roles.patrol_indices", raw_patrol_indices);
  node_->get_parameter(
    "nav.decision_config.patrol_preview_points", patrol_preview_points_);

  goal_points_ = buildGoalPoints(xs, ys, zs);
  patrol_indices_ = toSizeIndices(
    raw_patrol_indices, goal_points_.size(), "nav.point_roles.patrol_indices");
}
...
BT::NodeStatus SelectPatrolPathAction::tick()
{
  ...
  if (patrol_indices_.size() == 1) {
    nav_msgs::msg::Path path =
      buildPathFromIndices(goal_points_, {patrol_indices_.front()});
    setOutput("path", path);
  ...
  nav_msgs::msg::Path path =
    buildPathFromIndices(goal_points_, route_indices);
...
{
  factory.registerNodeType<nav2_plugins::SelectPatrolPathAction>("SelectPatrolPath");
}
