#include "nav2_plugins/bt/action/send_nav_through_poses.hpp"
...
: BT::SyncActionNode(name, config), node_(getNodeFromBlackboard(*this))
{
  ...
  node_->get_parameter("nav.decision_config.nav2_action_server", action_name_);
  node_->get_parameter(
    "nav.decision_config.goal_position_tolerance", path_compare_tolerance_);
  node_->get_parameter(
    "nav.decision_config.action_server_wait_timeout_s", action_server_wait_timeout_s_);
...
BT::NodeStatus SendNavThroughPosesAction::tick()
{
...

  {
    ...
    const bool same_path =
      pathEquivalent(active_path_, *path, path_compare_tolerance_);
    setOutput(
      "goal_succeeded",
...
{
  factory.registerNodeType<nav2_plugins::SendNavThroughPosesAction>(
    "SendNavThroughPoses");
}
