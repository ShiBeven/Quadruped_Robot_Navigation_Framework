  ...
  std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  move_pid_->setSumErrorLimit(min_max_sum_error_);
  heading_pid_->setSumErrorLimit(min_max_sum_error_);
}
  ...
  for (const auto & parameter : parameters) {
    ...
    if (type == ParameterType::PARAMETER_DOUBLE) {
      if (name == plugin_name_ + ".translation_kp") {
        ...
        if (move_pid_) move_pid_->setSumErrorLimit(min_max_sum_error_);
        if (heading_pid_) heading_pid_->setSumErrorLimit(min_max_sum_error_);
