...
: dt_(dt), max_(max), min_(min), kp_(kp), kd_(kd), ki_(ki), pre_error_(0), integral_(0), i_max_(1.0)
{
}
...
double PID::calculate(double set_point, double pv)
{
  ...
  // Integral term (clamp before use so anti-windup takes effect on the current cycle)
  integral_ += error * dt_;
  if (integral_ > i_max_) {
    integral_ = i_max_;
