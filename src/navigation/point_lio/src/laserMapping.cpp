...
// Resolve the base directory for saved PCD files: use the `pcd_save.pcd_save_dir` param
// when set, otherwise fall back to the compile-time ROOT_DIR/PCD (backward compatible).
// A trailing '/' is ensured so callers can append the file name directly.
static inline string get_pcd_save_dir()
{
  string dir = pcd_save_dir.empty() ? (string(ROOT_DIR) + "PCD/") : pcd_save_dir;
      ...
      *pcl_wait_save += *feats_down_world;
      ...
      if (!pcl_wait_save->empty() && pcd_save_interval > 0 && scan_wait_num >= pcd_save_interval) {
        ...
        string all_points_dir(
          get_pcd_save_dir() + string("scans_") + to_string(pcd_index) + string(".pcd"));
...
angvel_avr << imu_last.angular_velocity.x, imu_last.angular_velocity.y,
  ...
  if (!pcl_wait_save->empty() && pcd_save_en) {
    ...
    string all_points_dir(get_pcd_save_dir() + file_name);
    pcl::PCDWriter pcd_writer;
    pcd_writer.writeBinary(all_points_dir, *pcl_wait_save);
  }
  fout_out.close();
