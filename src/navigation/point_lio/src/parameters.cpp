...
// Output TF frame names. Defaults preserve the historical Point-LIO naming so existing
// setups keep working; override via ROS params to fit another robot's TF conventions.
std::string world_frame = "camera_init", body_frame = "body", aft_mapped_frame = "aft_mapped";
// Directory PCD scans are saved into. Empty => fall back to the compile-time ROOT_DIR.
...
void readParameters(std::shared_ptr<rclcpp::Node> & nh)
{
  ...
  try {
    ...
    nh->declare_parameter<std::string>("pcd_save.pcd_save_dir", "");
    nh->get_parameter("pcd_save.pcd_save_dir", pcd_save_dir);

    // Output TF frame names (defaults keep backward compatibility with legacy naming).
    nh->declare_parameter<std::string>("publish.world_frame", "camera_init");
    nh->get_parameter("publish.world_frame", world_frame);

    nh->declare_parameter<std::string>("publish.body_frame", "body");
    nh->get_parameter("publish.body_frame", body_frame);

    nh->declare_parameter<std::string>("publish.aft_mapped_frame", "aft_mapped");
    nh->get_parameter("publish.aft_mapped_frame", aft_mapped_frame);
