...
{
  // The chassis map only defines the x/y/yaw DOFs. Only those are filled; the remaining
  // Twist components stay zero. (Gimbal control is a separate concern and is not routed
  // through this cmd_vel Twist publisher — see axis_gimbal in the config, which needs a
  // dedicated topic/message to avoid conflicting with chassis yaw.)
